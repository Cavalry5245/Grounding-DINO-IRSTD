...
try:
    ...
    print("Install with: pip install peft>=0.6.0")
...
def get_args_parser():
    ...
    parser.add_argument('--lora_modules_to_save', type=str, nargs='+',
                        default=None,
                        help='Modules to save completely (not just LoRA weights)')
...
def find_all_linear_names(model, exclude_keywords=None):
    ...
    if exclude_keywords is None:
        exclude_keywords = ['class_embed', 'bbox_embed', 'label_enc']  # 通常不对这些层应用LoRA
...
def apply_lora(model, args, logger=None):
    """Apply LoRA to the model using peft library."""
    if not PEFT_AVAILABLE:
        raise RuntimeError("peft library is not installed. Install with: pip install peft>=0.6.0")
    ...
    if logger:
        logger.info("=" * 50)
        logger.info("LoRA Configuration (using peft):")
        ...
        logger.info(f"  - modules_to_save: {args.lora_modules_to_save}")
        logger.info("=" * 50)
...
def load_lora_model(base_model, lora_path, logger=None):
    ...
    if not PEFT_AVAILABLE:
        raise RuntimeError("peft library is not installed.")
...
def main(args):
    ...
    if args.use_lora:
        if not PEFT_AVAILABLE:
            raise RuntimeError(
                ...
                "Install with: pip install peft>=0.6.0"
            )
    ...
    data_loader_val = DataLoader(
        dataset_val, 4, sampler=sampler_val,
